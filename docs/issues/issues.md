...
- [ ] **V2-08** — Team Wars / Faction Wars par division — 🟡 [#90](https://github.com/igorms-pro/truegrynd/issues/90) · branche `feature/issue-90-team-wars` · migration **`023`**
...
- [x] **Règle d'entrée** : division par défaut `rookie` (signup + backfill) ; promotion auto 🟢 **V2-05** (one-way, trigger rating).
...
- [x] **Leaderboards** : division + faction + ville/pays (presets local / national / global).
...
- [x] **Produit** : permettre “Top Rookie Paris”, “Horde Regular France”, “Savage Global”. — 🟢 [#79](https://github.com/igorms-pro/truegrynd/issues/79) PR [#80](https://github.com/igorms-pro/truegrynd/pull/80)
- [x] **Profil** : ajouter ville/pays optionnels ou inférés prudemment (pas de géoloc invasive V2.1).
- [x] **DB / query** : index et filtres performants pour division + faction + location.
- [x] **UI leaderboard** : presets simples plutôt qu'un panneau de filtres monstrueux.
- [x] **Privacy** : ne pas afficher une localisation trop précise par défaut.

### V2-05. Truegrynd Rating

**Statut :** 🟢 **LIVRÉ** — [#81](https://github.com/igorms-pro/truegrynd/issues/81) fermée · PR [#82](https://github.com/igorms-pro/truegrynd/pull/82) mergée `main` · migration **`019`** (à appliquer prod si pas déjà fait)

- [x] **Produit** : rating global + axes `engine`, `power`, `strength`, `grit`, `consistency`.
- [x] **Algorithme V1** : percentiles validés + `challenges.rating_axis` ; pas d'Elo opaque.
- [x] **DB** : `profile_ratings` + `profile_rating_history` + trigger recalc sur score validé.
- [x] **Promotion division** : one-way Rookie → Regular → Savage → Elite (ferme dette V2-01).
- [x] **UI** : carte rating profil (own + public) + explication EN/FR.
- [x] **Tests** : `src/lib/rating/*` (percentiles, promotion, infer axis).
...
- [x] **UI** : `/app/profile/passport` mobile-first + lien depuis profil (CARDS reste sur profil).
- [x] **Share** : copie lien public `/app/u/[username]`.
- [x] **Privacy** : toggles masquer sections sur profil public (migration **`020`**).
...
**Statut :** 🟡 **EN COURS** — [#90](https://github.com/igorms-pro/truegrynd/issues/90) · branche `feature/issue-90-team-wars` · migration **`023`**

- [x] **Produit** : guerre collective hebdo : faction x division x challenge (fenêtre `weekly_challenges` active).
- [x] **Score équipe** : agrégat serveur top 10 contributions + bonus participation (RPC `get_faction_war_standings`).
- [x] **DB** : `faction_war_contributions` + trigger sur score validé + RPC standings / top contributors.
- [x] **UI** : Clan + page faction — scoreboard serveur, meta weekly/division, contribution perso.
- [x] **Fairness** : standings filtrés par division du viewer.
- [ ] **Finisher Card** : “I scored for Horde Rookie” (PR2 ou lot suivant).
- [ ] **Fallback legacy** : retirer heuristique client quand weekly inactive (optionnel).
...
| **V2 — Accessible competition**               | 🟢 V2-00–06 · **🟡 V2-07** [#85](https://github.com/igorms-pro/truegrynd/issues/85) PR4 [#89](https://github.com/igorms-pro/truegrynd/pull/89) · **🟡 V2-08** [#90](https://github.com/igorms-pro/truegrynd/issues/90) `feature/issue-90-team-wars` · migration **`023`** |
| **QA V1**                                     | 🟢 GO (juin 2026) — périmètre critique testé                                                                                                                                                                                                                              |

**Suite produit :** merger **PR4** [#89](https://github.com/igorms-pro/truegrynd/pull/89) → clôturer **V2-07** [#85](https://github.com/igorms-pro/truegrynd/issues/85) · livrer **V2-08** [#90](https://github.com/igorms-pro/truegrynd/issues/90) (PR1 migration **`023`** + Clan serveur ; finisher card en PR2). QA V2 globale en fin de lot V2.
