...
import {
  adminUpsertWeeklyChallenge,
  listApprovedChallengesForWeeklyPicker,
  listWeeklyChallengesForAdmin,
  type AdminWeeklyRow,
} from '@/features/admin/services/adminWeeklyChallenges';
...
const emptyLists = {
  rows: [] as AdminWeeklyRow[],
  challenges: [] as { id: string; title: string; is_official: boolean }[],
};

export function useAdminWeeklyScheduler(): {
  state: State;
  refetch: () => void;
  save: (input: {
    id?: string | null;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) {
          setState({ status: 'ready', rows, challenges, error: null });
        }
      } catch (e: unknown) {
        const message = e instanceof Error ? e.message : 'unknown';
        if (!cancelled) {
          setState({ status: 'error', ...emptyLists, error: message });
        }
      }
    })();
  ...
  const save = async (input: {
    ...
    setSaveError(null);
    try {
      await adminUpsertWeeklyChallenge(input);
      refetch();
    } catch (e: unknown) {
